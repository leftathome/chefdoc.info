class OpscodeRouter < YARD::Server::Router
  def docs_prefix; 'opscode' end
  def list_prefix; 'list/opscode' end
  def search_prefix; 'search/opscode' end
  ...
  def [](name)
    ...
    Dir.entries(path).map do |dir|
      ...
      if File.exist?(File.join(yfile, 'complete'))
        YARD::Server::LibraryVersion.new(name, dir, yfile, :opscode)
      else
        nil
      end
    end.compact
